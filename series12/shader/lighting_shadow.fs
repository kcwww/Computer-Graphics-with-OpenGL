...
uniform int blinn;
uniform sampler2D shadowMap; // depth map from light’s perspective

// 그림자가 있는지 확인하는 함수
// 그림자 맵을 사용하여 현재 픽셀이 그림자에 있는지 확인합니다.
float ShadowCalculation(vec4 fragPosLight, vec3 normal, vec3 lightDir) {
    ...
    // PCF filtering
    // we sample the depth map at 9 different positions around the current fragment
...
void main() {
    ...
    if (intensity > 0.0) {
        ...
        float shadow = ShadowCalculation(fs_in.fragPosLight, pixelNorm, lightDir); // 그림자가 있는지 확인
